...
module.exports.storage = async (answers) => {
  ...
  GROUP by sku.description, currency, labels
  HAVING total_cost <> 0
...
module.exports.service = async (answers) => {
  ...
  console.log(chalk.green('\nCost per service:\n'));
  rows.forEach((row) => {
    console.log(`${row.description}: ${chalk.yellow(`${row.total_cost} ${row.currency}`)}`);
  });
};

module.exports.compute = async (answers) => {
  const query = `SELECT
    ROUND(SUM(cost) + SUM(IFNULL((SELECT SUM(c.amount) FROM UNNEST(credits) c), 0)), 2) AS total_cost,
    currency,
    sku.description,
...
module.exports.gkeVsNonGkeCompute = async (answers) => {
  const query = `SELECT
    ...
    AS total_cost,
    currency,
    'goog-gke-node' IN (SELECT key FROM unnest(labels)) AS is_gke
    FROM
    \`${answers.dataset}.${answers.table}\`
  WHERE
    invoice.month=@invoice_month
